...
def week9(file_path):
    ...
    if last_column >= 8:
        ...
        for school in unique_schools:
            ...
            orig_week9_ws = wb['week9']
            new_week9_ws = new_wb.create_sheet(title='week9')
            
            for row in orig_week9_ws.iter_rows(values_only=True):
                new_week9_ws.append(row)
            
            # Convert the 'week1' sheet data into a table
            last_row_week9 = new_week9_ws.max_row
            last_column_week9 = new_week9_ws.max_column
            tbl_range_week9 = f"A1:{get_column_letter(last_column_week9)}{last_row_week9}"
            table_week9 = Table(displayName="Week9Table", ref=tbl_range_week9)
            
            # Add a table style for the 'week1' sheet
            style_week9 = TableStyleInfo(name="TableStyleMedium9", showFirstColumn=False,
                                        showLastColumn=False, showRowStripes=True, showColumnStripes=True)
            table_week9.tableStyleInfo = style_week9
            new_week9_ws.add_table(table_week9)
            
            # Save the new workbook in the school's folder with the name "week1.xlsx"
            new_file_path = os.path.join(folder_path, 'week9.xlsx')

...
def week8(file_path):
    ...
    if last_column >= 8:
        ...
        for school in unique_schools:
            ...
            orig_week8_ws = wb['week8']
            new_week8_ws = new_wb.create_sheet(title='week8')
            
            for row in orig_week8_ws.iter_rows(values_only=True):
                new_week8_ws.append(row)
            
            # Convert the 'week1' sheet data into a table
            last_row_week8 = new_week8_ws.max_row
            last_column_week8 = new_week8_ws.max_column
            tbl_range_week8 = f"A1:{get_column_letter(last_column_week8)}{last_row_week8}"
            table_week8 = Table(displayName="Week8Table", ref=tbl_range_week8)
            
            # Add a table style for the 'week1' sheet
            style_week8 = TableStyleInfo(name="TableStyleMedium9", showFirstColumn=False,
                                        showLastColumn=False, showRowStripes=True, showColumnStripes=True)
            table_week8.tableStyleInfo = style_week8
            new_week8_ws.add_table(table_week8)
            
            # Save the new workbook in the school's folder with the name "week1.xlsx"
            new_file_path = os.path.join(folder_path, 'week8.xlsx')

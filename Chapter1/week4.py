...
def week4(file_path):
    ...
    if last_column >= 8:
        ...
        for school in unique_schools:
            ...
            orig_week4_ws = wb['week4']
            new_week4_ws = new_wb.create_sheet(title='week4')
            
            for row in orig_week4_ws.iter_rows(values_only=True):
                new_week4_ws.append(row)
            
            # Convert the 'week1' sheet data into a table
            last_row_week4 = new_week4_ws.max_row
            last_column_week4 = new_week4_ws.max_column
            tbl_range_week4 = f"A1:{get_column_letter(last_column_week4)}{last_row_week4}"
            table_week4 = Table(displayName="Week4Table", ref=tbl_range_week4)
            
            # Add a table style for the 'week1' sheet
            style_week4 = TableStyleInfo(name="TableStyleMedium9", showFirstColumn=False,
                                        showLastColumn=False, showRowStripes=True, showColumnStripes=True)
            table_week4.tableStyleInfo = style_week4
            new_week4_ws.add_table(table_week4)
            
            # Save the new workbook in the school's folder with the name "week1.xlsx"
            new_file_path = os.path.join(folder_path, 'week4.xlsx')

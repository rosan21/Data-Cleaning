...
def week3(file_path):
    ...
    if last_column >= 8:
        ...
        for school in unique_schools:
            ...
            orig_week3_ws = wb['week3']
            new_week3_ws = new_wb.create_sheet(title='week3')
            
            for row in orig_week3_ws.iter_rows(values_only=True):
                new_week3_ws.append(row)
            
            # Convert the 'week1' sheet data into a table
            last_row_week3 = new_week3_ws.max_row
            last_column_week3 = new_week3_ws.max_column
            tbl_range_week3 = f"A1:{get_column_letter(last_column_week3)}{last_row_week3}"
            table_week3 = Table(displayName="Week3Table", ref=tbl_range_week3)
            
            # Add a table style for the 'week1' sheet
            style_week3 = TableStyleInfo(name="TableStyleMedium9", showFirstColumn=False,
                                        showLastColumn=False, showRowStripes=True, showColumnStripes=True)
            table_week3.tableStyleInfo = style_week3
            new_week3_ws.add_table(table_week3)
            
            # Save the new workbook in the school's folder with the name "week1.xlsx"
            new_file_path = os.path.join(folder_path, 'week3.xlsx')

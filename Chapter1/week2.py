...
def week2(file_path):
    ...
    if last_column >= 8:
        ...
        for school in unique_schools:
            ...
            orig_week2_ws = wb['week2']
            new_week2_ws = new_wb.create_sheet(title='week2')
            
            for row in orig_week2_ws.iter_rows(values_only=True):
                new_week2_ws.append(row)
            
            # Convert the 'week1' sheet data into a table
            last_row_week2 = new_week2_ws.max_row
            last_column_week2 = new_week2_ws.max_column
            tbl_range_week2 = f"A1:{get_column_letter(last_column_week2)}{last_row_week2}"
            table_week2 = Table(displayName="Week2Table", ref=tbl_range_week2)
            
            # Add a table style for the 'week1' sheet
            style_week2 = TableStyleInfo(name="TableStyleMedium9", showFirstColumn=False,
                                        showLastColumn=False, showRowStripes=True, showColumnStripes=True)
            table_week2.tableStyleInfo = style_week2
            new_week2_ws.add_table(table_week2)
            
            # Save the new workbook in the school's folder with the name "week1.xlsx"
            new_file_path = os.path.join(folder_path, 'week2.xlsx')
